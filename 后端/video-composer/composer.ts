...
export function composeErrorMessage(e: { code?: number | string; killed?: boolean; signal?: string; stderr?: string; message?: string }): string | null {
  if (e.killed || e.signal === "SIGTERM") return "视频合成超时（已超过 10 分钟）——可能分镜过多或机器繁忙，请减少分镜或降到「快速」画质重试";
  const msg = `${e.stderr || ""} ${e.message || ""}`;
  if (e.signal === "SIGKILL" || e.code === 137) return "FFmpeg 被系统终止（SIGKILL）——通常是当前 ffmpeg 二进制不可运行、被 macOS 拦截，或资源占用过高。请安装/切换到可运行且带 drawtext 的 FFmpeg 后重试";
  if (/no space left|ENOSPC/i.test(msg)) return "磁盘空间不足，无法写出成片——请清理磁盘后重试";
  if (/No such filter:\s*'?drawtext'?|Filter not found/i.test(msg)) return "当前 FFmpeg 缺少 drawtext 字幕滤镜，无法烧录字幕——macOS 推荐安装 ffmpeg-full，或设置 FFMPEG_PATH 指向带 drawtext/libass 的 ffmpeg";
  if (/command not found|ENOENT|not found/i.test(msg) && /ffmpeg/i.test(msg)) return "未找到可用 FFmpeg——请安装带 drawtext/libass 的 FFmpeg，或设置 FFMPEG_PATH 指向可执行文件";
  return null;
}
