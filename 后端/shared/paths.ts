...
export function getDataDir(): string {
  return process.env.APP_DATA_DIR || join(/* turbopackIgnore: true */ process.cwd(), "data");
}

/** 迁移 SQL 目录（只读资源）。Electron 打包时指向 resources 内的 drizzle */
export function getMigrationsDir(): string {
  return process.env.APP_MIGRATIONS_DIR || join(/* turbopackIgnore: true */ process.cwd(), "drizzle");
}

/** 上传素材根目录 data/uploads */
export function getUploadsDir(): string {
  return join(/* turbopackIgnore: true */ getDataDir(), "uploads");
}

/** 合成输出根目录 data/output */
export function getOutputDir(): string {
  return join(/* turbopackIgnore: true */ getDataDir(), "output");
}
