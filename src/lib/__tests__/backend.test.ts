...
import { buildUserPrompt, buildBatchPrompt } from "@backend/script-engine/prompts";
import type { ScriptGenerationInput } from "@backend/script-engine/prompts";
import { extractJSON, parseScriptResponse } from "@backend/script-engine/generator";
import { buildComposeCommand, resolveChineseFontFamily, wrapCaption, composeErrorMessage, buildDrawtext, type ComposeConfig } from "@backend/video-composer/composer";
...
describe("composeErrorMessage（ffmpeg 合成错误归类）", () => {
  ...
  it("FFmpeg 被系统杀掉(SIGKILL/137) → 可操作提示", () => {
    expect(composeErrorMessage({ signal: "SIGKILL" })).toMatch(/FFmpeg|SIGKILL/);
    expect(composeErrorMessage({ code: 137 })).toMatch(/FFmpeg|SIGKILL/);
  });
  it("缺少 drawtext 字幕滤镜 → 可操作提示", () => {
    expect(composeErrorMessage({ stderr: "No such filter: 'drawtext'\nError : Filter not found" })).toMatch(/drawtext/);
  });
  it("其它错误 → null（原样抛）", () => {
