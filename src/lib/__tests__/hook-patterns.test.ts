...
import { selectHookPatterns, buildHookGuidance, HOOK_PATTERNS, hookPatternName } from "@backend/script-engine/hook-patterns";
...
describe("selectHookPatterns", () => {
  ...
  it("hookPatternName 可把 id 映射成人类可读名称", () => {
    expect(hookPatternName("visual_shock")).toBe("视觉冲击");
    expect(hookPatternName("unknown_hook")).toBe("unknown_hook");
  });
});
...
describe("buildUserPrompt 已接入钩子指引（集成锁）", () => {
  ...
  it("历史效果反馈会注入在钩子指南之前", () => {
    ...
    expect(prompt.indexOf("历史转化数据反馈")).toBeLessThan(prompt.indexOf("黄金3秒钩子"));
  });
});
