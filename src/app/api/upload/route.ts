...
export async function POST(req: NextRequest) {
  // 鉴权：必须登录商家，且只能往自己拥有的项目目录写（防匿名写盘 DoS / 向他人项目投毒）
  const auth = await requireMerchant(req);
  if ("error" in auth) return auth.error;

  const limit = consumeExpensiveRouteRateLimit(req, auth.merchant.id, "upload:project", {
    merchantBurst: 8,
    ipBurst: 24,
  ...
  // 整批文件先通过统一校验，再进行任何目录创建或写盘，避免后续文件无效留下半批上传。
