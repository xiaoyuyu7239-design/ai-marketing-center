...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limit = consumeExpensiveRouteRateLimit(req, auth.merchant.id, "project:carousel", EXPENSIVE_RATE_LIMIT_PRESETS.cpu);
  if (!limit.allowed) return rateLimitResponse(limit, "图文卡片生成过于频繁，请稍后再试");
